...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
        ...
        list_iAS = unordered;
        for( int index = 0
           ; index < list_iAS.size() - 1
           ; index++
           ){int champIndex = champIndex(index);
             list_iAS.set(champIndex
                        ,list_iAS.set(index, list_iAS.get(champIndex)));
            System.out.println("After index " + index + ": " + list_iAS);
           }
    }
     ...
     */
     ...
     private int champIndex(int startingAt) {
       int min = startingAt;
       for( int index = startingAt + 1
          ; index < list_iAS.size()
          ; index++
          ){if(list_iAS.get(index) < list_iAS.get(min)) min = index;}
       return min;
     }
